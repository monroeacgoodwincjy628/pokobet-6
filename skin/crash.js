...
flashEl.textContent=text;flashEl.className="arc-flash-8fe46bd7"+
(kind==="win"?"arc-flash-win":"arc-flash-bust");}
function hideFlash(){if(flashEl){flashEl.className="arc-flash-8fe46bd7";flashEl.textContent="";}}
function shakeScreen(){if(!screenBox){return;}
screenBox.classList.add("arc-shake-8fe46bd7");window.setTimeout(function(){screenBox.classList.remove("arc-shake-8fe46bd7");},500);}
function pushHistory(value){state.history.unshift(value);if(state.history.length>HISTORY_SIZE){state.history.pop();}
if(!histEl){return;}
histEl.textContent="";var i;var li;for(i=0;i<state.history.length;i+=1){li=document.createElement("li");li.className="arc-hist-8fe46bd7"+
...
writeBet(bet);state.bet=bet;state.balance-=bet;updateBalance();state.crashAt=rollCrashPoint();state.cashed=false;state.cashMult=0;state.mult=1;state.tick=0;state.phase="flying";state.startStamp=window.performance.now();setControlsLocked(true);actionBtn.textContent="CASH OUT";actionBtn.classList.add("arc-launch-live-8fe46bd7");hideFlash();setMsg("CLIMBING... CASH OUT ANY TIME");beep(220,0.09);state.rafId=window.requestAnimationFrame(frame);}
